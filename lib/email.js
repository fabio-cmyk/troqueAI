...
const TEMPLATES = {
  ...
  produto_recebido: {
    subject: 'Produto recebido - Solicitacao #{{protocolo}}',
    html: `
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #6366f1, #8b5cf6); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
          <h1 style="color: white; margin: 0;">Produto Recebido!</h1>
        </div>
        <div style="padding: 30px; background: #f9fafb; border-radius: 0 0 12px 12px;">
          <p>Ola <strong>{{cliente_nome}}</strong>,</p>
          <p>Recebemos o produto da sua solicitacao <strong>#{{protocolo}}</strong>.</p>
          <p>Estamos analisando e em breve voce recebera o retorno com o resultado.</p>
          <hr style="border: 1px solid #e5e7eb; margin: 20px 0;">
          <p style="color: #6b7280; font-size: 14px;">Obrigado pela paciencia!</p>
        </div>
      </div>
    `
  },
  solicitacao_rejeitada: {
    subject: 'Solicitacao #{{protocolo}} nao aprovada',
    html: `
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #ef4444, #dc2626); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
          <h1 style="color: white; margin: 0;">Solicitacao Nao Aprovada</h1>
        </div>
        <div style="padding: 30px; background: #f9fafb; border-radius: 0 0 12px 12px;">
          <p>Ola <strong>{{cliente_nome}}</strong>,</p>
          <p>Infelizmente sua solicitacao <strong>#{{protocolo}}</strong> nao foi aprovada.</p>
          {{#motivo_rejeicao}}
          <div style="background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 16px; margin: 16px 0;">
            <p style="margin: 0; color: #991b1b;"><strong>Motivo:</strong> {{motivo_rejeicao}}</p>
          </div>
          {{/motivo_rejeicao}}
          <p>Se tiver duvidas, entre em contato com a loja <strong>{{loja_nome}}</strong>.</p>
          <hr style="border: 1px solid #e5e7eb; margin: 20px 0;">
          <p style="color: #6b7280; font-size: 14px;">Pedimos desculpas pelo inconveniente.</p>
        </div>
      </div>
    `
  },
