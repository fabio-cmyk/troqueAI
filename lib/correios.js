...
async function gerarPostagemReversa(tenantConfig, dadosSolicitacao, servico = 'pac') {
  ...
  // Se Correios não configurado, gerar código simulado
  if (!correios_usuario || !correios_token || !correios_cartao_postagem) {
    return gerarCodigoSimulado(tenantConfig);
  }
  ...
  try {
    ...
    return gerarCodigoSimulado(tenantConfig, err.message);
  }
}
...
module.exports = { autenticar, verificarCredenciais, gerarPostagemReversa, calcularFreteReverso };
